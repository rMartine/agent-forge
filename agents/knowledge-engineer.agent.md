...
1. **Audit BOTH agent trees.** The roster is dual-hosted:
   - **Copilot source:** `agents/*.agent.md` with Copilot frontmatter (`tools: [toolset-name]`, `agents: [...]`, `handoffs: [...]`, `user-invocable`, `disable-model-invocation`). Check that `agents:` allowlists match the body's delegation/routing text. Check `handoffs:` targets exist.
   - **Claude Code derivation:** `.claude/agents/<division>/*.md` with CC frontmatter (`name`, `description`, `tools: <comma list>`, `model`, `skills: [...]`). 18 of 21 specialists are auto-generated by `scripts/port-to-claude-code/port.mjs` from the Copilot source. The 3 orchestrators (`cto`, `principal-engineer`, `creative-director`) are HAND-MAINTAINED — verify those have NOT drifted from the Copilot source bodies (allowlist, direct reports, routing rules).
2. **Audit the workspace manual.** Read `CLAUDE.md` (workspace root). Flag any contradictions with current agent definitions, or any constraints that conflict with what an agent body says.
3. **Audit the manifest.** Read `agent-forge.manifest.jsonc`. Confirm every roster entry references a real agent file and an existing `toolset`. Confirm `skills:` and `instructions:` arrays match the files actually on disk.
4. **Audit the port script config maps.** Read `scripts/port-to-claude-code/port.mjs`. Verify `MODELS_MAP`, `SKILLS_MAP`, `MCP_MAP`, and `TOOLSET_MAP` reference existing agents (no entries for deleted agents) and that skill / MCP names are valid (built-in, plugin-prefixed, or in the project's `skills/` folder).
5. **Cross-check with KB.** Query the knowledge base for any entries tagged `agent-routing`, `frontmatter`, or `orchestration`. Confirm each documented anti-pattern is still prevented by the current configuration.
6. **Produce a findings report.** Hand off to `@cto` with: list of inconsistencies, proposed fixes, and severity. CTO routes implementation to `@principal-engineer`.
7. **Log the pass.** Append a one-line entry to `project_docs/knowledge/agent-modifications.md` recording date, scope, and outcome.
